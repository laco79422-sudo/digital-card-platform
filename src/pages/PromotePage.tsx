...
export function PromotePage() {
  ...
  const applyPromotion = async (card: BusinessCard) => {
    ...
    if (ownerUserId === user.id) {
      setMessage("내 명함에는 고객 유입 링크 신청을 할 수 없습니다.");
      return;
    }
    ...
    if (existing) {
      setMessage("이미 고객 유입 링크를 신청한 명함입니다. 내 공간에서 승인 상태를 확인해 주세요.");
      return;
    }
    ...
    setMessage("고객 유입 링크 신청이 접수되었습니다. 명함 오너 승인 후 전용 링크를 받을 수 있어요.");
  };
    ...
    <div className={cn(layout.page, "py-10 sm:py-12")}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-base leading-relaxed text-slate-600">
            고객 유입 링크로 연결할 명함을 선택하고 신청하면, 승인 후 내 전용 링크를 받을 수 있어요.
          </p>
        </div>
              ...
              <CardHeader>
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs font-semibold text-brand-700">총 유입 링크 신청 수 {applicationsCount}</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
      ...
      {promotableCards.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-12 text-center">
          <p className="text-lg font-bold text-slate-900">아직 유입 링크 신청 가능한 명함이 없습니다.</p>
          <p className="mt-2 text-sm text-slate-500">명함 오너가 고객 유입 링크 기능을 켜면 이곳에 표시됩니다.</p>
        </div>
      ) : null}
    </div>
  );
